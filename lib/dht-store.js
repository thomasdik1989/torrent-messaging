...
export function createDHT() {
  return new Promise((resolve, reject) => {
    ...
    setTimeout(() => {
      if (!resolved) {
        resolved = true;
        console.log(`Bootstrap timeout, connected to ${nodeCount} nodes`);
        if (nodeCount > 0) {
          resolve(dht);
        } else {
 ...
 */
export function getMutable(dht, publicKey, timeout = 30000) {
  return new Promise((resolve) => {
    ...
    dht.get(publicKey, (err, result) => {
      ...
      if (err || !result) {
        // No result found, will resolve with null on timeout or if no better result comes
        return;
      }
      ...
      // Resolve with the result after a short delay to allow for better results
      setTimeout(() => {
        if (!resolved) {
          resolved = true;
